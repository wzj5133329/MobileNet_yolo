...
bool ReadImageToDatum(const string& filename, const int label,
  ...
  if (cv_img.data) {
    ...
    CVMatToDatum(cv_img, datum);    //单用这种方式制作数据集会非常占空间
    datum->set_label(label);
    return true;
  } else {
    return false;
    ...
    if (v1.first == "object") {
      ...
      BOOST_FOREACH(ptree::value_type &v2, object.get_child("")) {
        ...
        if (v2.first == "name") {
          ...
          LOG_IF(WARNING,2>1) <<"name"<<name;
          if (name_to_label.find(name) == name_to_label.end()) {
            LOG(FATAL) << "Unknown name: " << name;
          }
          int label = name_to_label.find(name)->second;
          LOG_IF(WARNING,2>1) <<"label"<<label;
          bool found_group = false;
          LOG_IF(WARNING,2>1) <<"anno_datum->annotation_group_size()"<<anno_datum->annotation_group_size();
          ...
          LOG_IF(WARNING,2>1) <<"instance_id"<<instance_id;
